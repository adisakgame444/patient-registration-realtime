...
//   // ✅ แก้ไข: เพิ่ม e.stopPropagation() เพื่อป้องกัน Event Bubbling
//   const handleSelect = (e: React.MouseEvent, val: string) => {
//     e.stopPropagation(); // หยุดไม่ให้เหตุการณ์คลิกวิ่งไปหาตัวแม่
//     setValue(name, val, {
//       shouldValidate: true,
//       shouldDirty: true,
//       shouldTouch: true,
...
//             selectedValue || isOpen ? "text-indigo-600" : "text-slate-400"
//           }`}
//         >
//           <Icon size={18} />
//         </div>
...
//         {/* ✅ เมนูตัวเลือก: ใส่ z-[100] เพื่อให้อยู่บนสุดเสมอ */}
//         {isOpen && (
//           <div className="absolute left-0 right-0 top-full z-[100] mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl p-1.5 animate-in fade-in slide-in-from-top-2 duration-200">
//             {options.map((opt) => (
//               <div
//                 key={opt.value}
//                 onClick={(e) => handleSelect(e, opt.value)} // ส่ง Event เข้าไปด้วย
//                 className={`px-4 py-3 cursor-pointer rounded-xl transition-all flex items-center justify-between ${
